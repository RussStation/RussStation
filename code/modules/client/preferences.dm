		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("hotkeys")
					hotkeys = !hotkeys
					if(hotkeys)
						bindings.bind_movement() //honk --Custom Keybindings
						winset(user, null, "input.focus=true input.background-color=[COLOR_INPUT_DISABLED] mainwindow.macro=default")//honk --Custom Kebyindings
					else
						bindings.unbind_movement() //honk --Custom Keybindings
